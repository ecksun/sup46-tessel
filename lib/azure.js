...
Azure.prototype.getStatus = function getStatus(cb) {
    http.get('https://checkhealth.azurewebsites.net/status', function (res) {
        // console.log('http get compelted');
        res.on('data', function (data) {
            var jsonData = JSON.parse(data);
            // console.log('# received', jsonData);
            cb(null, jsonData.fixing);
        });
        res.on('end', function () {
            // console.log('res end');
            // setImmediate(start);
...
Azure.prototype.buttonPress = function buttonPress() {
    ...
    req.on('connect', function(e,d) {
      console.log('post connect', e, d);
    });
